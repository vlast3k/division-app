...
class DivisionGame {
    ...
    calculatePoints(elapsedSeconds) {
        ...
        // 0 to halfTime: 10 points
        // halfTime to maxTime: linearly decreases from 10 to 5
        // maxTime+: 5 points
        
        if (elapsedSeconds <= halfTime) {
            return 10;
        } else if (elapsedSeconds <= maxTime) {
            // Linear interpolation: 10 -> 5 over halfTime seconds
    ...
    setupBackButtonHandler() {
        // Add a history state when game starts
        window.history.pushState({ gameScreen: true }, '');
    ...
    startQuestionTimer() {
        ...
        this.questionTimerInterval = setInterval(() => {
            ...
            // Update progress bar (maxTime seconds total)
    ...
    saveToLeaderboard(result) {
        const key = `leaderboard_${this.difficulty}`;
    ...
    renderLeaderboard() {
        ...
        this.leaderboardDifficultyEl.textContent = this.difficultySettings[this.difficulty].name;
    ...
    resetGame() {
        ...
        if (window.history.state && window.history.state.gameScreen) {
            window.history.back();
        }
    }
}
